...
// Smooth shading - normal interpolated across triangle
in vec3 vNormal;
in vec2 vUV;
in vec3 vWorldPos;
...
uniform bool uHasTexture;         // Whether object has texture
uniform sampler2D uTexture;       // Diffuse texture
uniform bool uUseAlphaTest;       // Whether to use alpha testing (for transparent leaves)
uniform float uAlphaCutoff;       // Alpha cutoff value (default 0.5)
...
void main() {
    ...
    if (uHasTexture) {
        ...
        // Alpha testing for transparent textures (like leaves)
        if (uUseAlphaTest && baseColor.a < uAlphaCutoff) {
            discard;  // Discard fully transparent fragments
        }
    } else {
        baseColor = vec4(uColor, uAlpha);
    }
